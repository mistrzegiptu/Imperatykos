...
{
    ListElement *temp = safe_malloc(sizeof(ListElement));
    temp->data = data;
...
ListElement* find_insertion_point(const List *p_list, ListElement *p_element)
{
    ...
    while(temp != NULL)
    {
        if(p_list->compare_data(temp->data, p_element->data) > 0)
            return temp;
        temp = temp->next;
    }
...
{
    if(p_list->head == NULL || p_data == NULL)
        return;
    ...
    while(iter != NULL)
    {
        if(p_list->compare_data(iter->data, p_data) > 0)
            push_after(p_list, p_data, iter);
